...
# CTANLoadOut.py
# (C) Günter Partosch, 2021/2022/2023/2024
...
programversion    = "1.55.3"
programdate       = "2024-07-20"
...
delete_temporary_file  = True                # flag: in remove_LaTeX_file and remove_other_file

err_mode          = "[CTANLoadOut] Warning: '{0} {1}' changed to '{2}' (due to {3})"
latex_files       = [".aux", ".bib", ".ilg", ".log", ".idx", ".ilg", ".ind", ".out", ".tex", ".pdf",
                     ".stat", ".tap", ".top", ".xref"]
...
author_text               = "[CTANLoadOut] Flag: Show author of the program and exit."                # option -a
download_text             = "[CTANLoad] Flag: Download associated documentation files [PDF]."         # option -f
integrity_text            = "[CTANLoad, check] Flag: Check the integrity of the 2nd .pkl file."       # option -c

lists_text                = "[CTANLoad, Check] Flag: Generate some special lists and prepare files for CTANOut."
                                                                                                      # option -l
make_output_text          = "[CTANLoadOut] Flag: Do not activate CTANLoad."                           # option -mo
no_files_text             = "[CTANOut] Flag: Do not generate output files."                           # option -nf
pdf_output_text           = "[CTANOut] Flag: Generate PDF output viua LuaLaTeX."                      # option -p

regenerate_text           = "[CTANLoad, check] Flag: Regenerate the two pickle files."                # option -r
statistics_text           = "[CTANLoadOut] Flag: statistics on terminal."                             # option -stat
topics_text               = "[CTANOut] Flag: Generate topic lists [meaning of topics + cross-reference (topics/packages, authors/packages); only for -m LaTeX]."
                                                                                                      # option -mt
verbose_text              = "[CTANLoadOut] Flag: Output is verbose."                                  # option -v
version_text              = "[CTANLoadOut] Flag: Show version of the program and exit."               # option -V

btype_text                = "[CTANOut] Type of BibLaTex entries to be generated [valid only for '-m BibLaTeX'/'--mode BibLaTeX']"
                                                                                                      # option -b
direc_text                = "[CTANLoad and CTANOut] OS folder (directory) for input and output files" # option -d
mode_text                 = "[CTANOut] Target format"                                                 # option -m
number_text               = "[CTANLoad] Maximum number of XML and PDF file downloads"                 # option -n
output_text               = "[CTANLoad and CTANOut] Generic name for output files [without extensions]"
                                                                                                      # option -o
skip_text                 = "[CTANOut] Skip specified CTAN fields."                                   # option -s
skip_biblatex_text        = "[CTANOut] Skip specified BibLaTeX fields."                               # option -sb
timeout_text              = "[CTANLoad and CTANOut] default timeout (sec) for subprocesses"           # option -tout

program_text              = """Combines the tasks of CTANLoad  and CTANOut:
CTANLoad: Loads XLM and PDF documentation files from CTAN a/o generates some special lists,
...
parser                    = argparse.ArgumentParser(formatter_class = argparse.RawDescriptionHelpFormatter,
                                 description     = "{0}\nVersion: {1} ({2})\n\n{3}  ".format(programname, programversion, programdate, program_text),
                                 prog            = programname,
...
group4.add_argument("-m", "--mode",                        # option -m/--mode
                    ...
                    choices = ["LaTeX", "latex", "tex", "RIS", "ris", "plain", "txt", "BibLaTeX",
...
set_load         = {'-A', '--author_template', '-Al', '--author_load_template', '-L', '--license_template',
                    '-Ll', '--license_load_template', '-f', '--download_files', '-k', '--key_template',
                    '-kl', '--key_load_template', '-n', '--number', '-t', '--template', '-tl',
                    '--template_load', '-dbg', '--debugging', '-y', '--year_template', '-yl', '--year_load_template'}
set_check        = {'-c','--check_integrity', '-l','--lists'}
set_output       = { '-A', '--author_template', '-Ao', '--author_out_template', '-b', '--btype', '-k',
                     '--key_template', '-ko', '--key_out_template', '-L', '--license_template', '-Lo',
                     '--license_out_template', '-m', '--mode', '-mo', '--make_output', '-mt', '--make_topics',
                     '-s', '--skip', '-sb', '--skip_biblatex' '-t', '--template', '-to', '--template_out',
                     '-dbg', '--debugging', '-nf', 'no_files', '-y', '--year_template', '-yo', '--year_out_template' }
...
def fold(s):                                               # function fold: auxiliary function: shortens/foldens long option lists for output
    ...
    if delete_temporary_file:
        if t in latex_files:
            if path.exists(args.output_name + t):
                ...
                if verbose:
                    print("[CTANLoadOut] Warning: LaTeX file '{}' removed".format(args.output_name + t))
            else:
                pass
    ...
    if delete_temporary_file:
        if t in other_files:
            if path.exists(args.output_name + t):
                os.remove(args.output_name + t)
                if verbose:
                    print("[CTANLoadOut] Warning: file '{}' removed".format(args.output_name + t))
            else:
                pass
...
def func_call_load():                                      # function func_call_load(): CTANLoad is processed.
    ...
    try:
        with TemporaryFile("r+", encoding="utf-8", errors="ignore") as f:
                                                           # temporary file
            process_load = subprocess.run(call_load, check=True, timeout=timeout10, encoding="utf-8",
        ...
        if verbose:
            print("[CTANLoadOut, load] Error: called process '{0}' not found,".format(call_load[1]), sys.exc_info()[0])
        sys.exit("[CTANLoadOut, load] Error: program terminated")
                                                           # program terminated    
    except FileNotFoundError as exc:                       # file not found
        if verbose:
            print("[CTANLoadOut, load] Error: file '{0}' not found".format(call_load[0]), exc)
        sys.exit("[CTANLoadOut, load] Error: program terminated")
                                                           # program terminated
        ...
        if verbose:
            print("[CTANLoadOut, load] Error: any unspecified error", sys.exc_info())
        sys.exit("[CTANLoadOut, load] Error: program terminated")
                                                           # program terminated
    if verbose:
...
def func_call_check():                                    # function func_call_check(): CTANLoad (Check) is processed.
    ...
    try:
        with TemporaryFile("r+", encoding="utf-8") as f:   # temporary file
            process_check  = subprocess.run(call_check, check=True, timeout=timeout5, stdout=f,
        ...
        if verbose:
            print("[CTANLoadOut, check] Error: called process '{0}' not found,".format(call_check[1]),
                  sys.exc_info()[0])
        sys.exit("[CTANLoadOut, check] Error: program terminated")
                                                           # program terminated    
    except FileNotFoundError as exc:                       # file not found
        if verbose:
            print("[CTANLoadOut, check] Error: file '{0}' not found".format(call_check[0]), exc)
        sys.exit("[CTANLoadOut, check] Error: program terminated")
                                                           # program terminated
    except subprocess.TimeoutExpired as exc:               # timeout
        if verbose:
        ...
        if verbose:
            print("[CTANLoadOut, check] Error: any unspecified error", sys.exc_info())
        sys.exit("[CTANLoadOut, check] Error: program terminated")
                                                           # program terminated
    if verbose:
...
def func_call_regeneration():                              # function func_call_regeneration(): CTANLoad (Regeneration) is processed.
    ...
    try:
        with TemporaryFile("r+", encoding="utf-8") as f:   # temporary file
            process_regeneration = subprocess.run(call_regeneration, check=True, timeout=timeout10,
                                                  stdout=f, stderr=subprocess.PIPE, universal_newlines=True)
            f.seek(0)                                      # rewind file
            for line in f.readlines():                     # line by line
                print(line, end=empty)
        ...
        if verbose:
            print("[CTANLoadOut, regeneration] Error: called process '{0}' not found,".format(call_regeneration[1]),
                  sys.exc_info()[0])
        sys.exit("[CTANLoadOut, regeneration] Error: program terminated")
                                                           # program terminated    
    except FileNotFoundError as exc:                       # file not found
        if verbose:
            print("[CTANLoadOut, regeneration] Error: file '{0}' not found".format(call_regeneration[0]), exc)
        sys.exit("[CTANLoadOut, regeneration] Error: program terminated")
                                                           # program terminated
    except subprocess.TimeoutExpired as exc:               # timeout
        if verbose:
        ...
        if verbose:
            print("[CTANLoadOut, regeneration] Error: any unspecified error", sys.exc_info())
        sys.exit("[CTANLoadOut, regeneration] Error: program terminated")
                                                           # program terminated
    if verbose:
        print("\n" + "[CTANLoadOut, regeneration] Info: CTANLoad (Regeneration) completed")
...
def func_call_output():                                    # function func_call_output(): CTANOut is processed.
    ...
    try:
        with TemporaryFile("r+", encoding="utf-8", errors="ignore") as f:
                                                           # temporary file
            process_out = subprocess.run(call_output, check=True, timeout=timeout, stdout=f,
        ...
        if verbose:
            print("[CTANLoadOut, output] Error: called process '{0}' not found,".format(call_output[1]),
                  sys.exc_info()[0])
        sys.exit("[CTANLoadOut, output] Error: program terminated")
                                                           # program terminated    
    except FileNotFoundError as exc:                       # file not found
        if verbose:
            print("[CTANLoadOut, output] Error: file '{0}' not found".format(call_output[0]), exc)
        sys.exit("[CTANLoadOut, output] Error: program terminated")
                                                           # program terminated
    except subprocess.TimeoutExpired as exc:               # timeout
        if verbose:
        ...
        if verbose:
            print("[CTANLoadOut, output] Error: any unspecified error", sys.exc_info())
        sys.exit("[CTANLoadOut, output] Error: program terminated")
                                                           # program terminated
    if verbose:
...
def func_call_compile():                                   # Compiles the generated LaTeX file
    ...
    if path.exists(file_name):
        if path.getsize(file_name) > 3000:
            ...
            if verbose:
                print("[CTANLoadOut, compilation] Info: Program call:", call_compile)
                
            startcompiletotal   = time.time()                     # sets begin of total time
            startcompileprocess = time.process_time()             # sets begin of process time
          
            try:
                process_compile1      = subprocess.run(call_compile, timeout=timeout10, check=True, 
                                                       capture_output=True)
                ...
                if len(compile1_errormessage) > 0:
                    if verbose:
                        print("[CTANLoadOut, compilation] Error: error in compilation")
                    sys.exit()
                else:
                    if verbose:
                        print("[CTANLoadOut, compilation] Info: more information in '{0}'".format(file_name_log))
                        print("[CTANLoadOut, compilation] Info: Compilation OK")
##                process_compile1      = subprocess.run(call_compile, timeout=timeout10, check=True, encoding="utf-8",
##                                                       capture_output=True, universal_newlines=True, errors="ignore")
            except subprocess.CalledProcessError as exc:           # process not found
                if verbose:
                    print("[CTANLoadOut, compilation] Error: called process '{0}' not found,".format(call_compile[1]),
                          sys.exc_info()[0])
                sys.exit("[CTANLoadOut, compilation] Error: program terminated")
                                                                   # program terminated    
            except FileNotFoundError as exc:                       # file not found
                if verbose:
                    print("[CTANLoadOut, compilation] Error: file '{0}' not found".format(call_compile[0]), exc)
                sys.exit("[CTANLoadOut, compilation] Error: program terminated")
                                                                   # program terminated
            except subprocess.TimeoutExpired as exc:               # timeout
                if verbose:
                    print("[CTANLoadOut, compilation] Error: timeout error", timeout10)
                sys.exit("[CTANLoadOut, compilation] Error: program terminated")
                                                                   # program terminated
            except KeyboardInterrupt as exc:                       # keyboard interrupt
                if verbose:
                ...
                if verbose:
                    print("[CTANLoadOut, compilation] Error: any unspecified error", sys.exc_info())
                sys.exit("[CTANLoadOut, compilation] Error: program terminated")
                                                                   # program terminated
            ...
            if statistics:                                         # outputs the compilation statistics
                ...
                print("total time (compilation): ".ljust(left + 3), str(round(endcompiletotal-startcompiletotal, 2)).rjust(pp), "s")
                print("process time (compilation): ".ljust(left + 3), str(round(endcompileprocess-startcompileprocess, 2)).rjust(pp), "s")
...
# ...................................................................
            ...
            try:
                process_compile2      = subprocess.run(call_compile, timeout=timeout10, check=True, 
                                                       capture_output=True)
...
# ...................................................................
            ...
            try:
                process_index      = subprocess.run(call_index, timeout=timeout, check=True,
                                                    capture_output=True, universal_newlines=True)
                index_errormessage = process_index.stderr          # possible error message
                index_message      = process_index.stdout
            except subprocess.CalledProcessError as exc:           # process not found
                if verbose:
                    print("[CTANLoadOut, index] Error: called process '{0}' not found,".format(call_index[1]),
                          sys.exc_info()[0])
                sys.exit("[CTANLoadOut, index] Error: program terminated")
                                                                   # program terminated    
            except FileNotFoundError as exc:                       # file not found
                if verbose:
                    print("[CTANLoadOut, index] Error: file '{0}' not found".format(call_index[0]), exc)
                sys.exit("[CTANLoadOut, index] Error: program terminated")
                                                                   # program terminated
            except subprocess.TimeoutExpired as exc:               # timeout
                if verbose:
                ...
                if verbose:
                    print("[CTANLoadOut, index] Error: any unspecified error", sys.exc_info())
                sys.exit("[CTANLoadOut, index] Error: program terminated")
                                                                   # program terminated
                
            if verbose:
                print("[CTANLoadOut, index] Info: more information in '{0}'".format(file_name_ilg))
            ...
            if statistics:                                         # outputs the compilation statistics
                ...
                print("total time (index generation): ".ljust(left + 3), str(round(endcompiletotal-startcompiletotal, 2)).rjust(pp), "s")
                print("process time (index generation): ".ljust(left + 3), str(round(endcompileprocess-startcompileprocess, 2)).rjust(pp), "s")
...
# ...................................................................
            ...
            try:
                process_compile3      = subprocess.run(call_compile, timeout=timeout10, check=True, 
                                                       capture_output=True)
                ...
                if len(compile3_errormessage) > 0:
                    ...
                    if verbose:
                        print("[CTANLoadOut, compilation] Info: more information in '{0}'".format(file_name_log))
                        print("[CTANLoadOut, compilation] Info: result in '" + direc + output_name + ".pdf'")
                        print("[CTANLoadOut, compilation] Info: Compilation OK")
            except subprocess.CalledProcessError as exc:           # process not found
                if verbose:
                    print("[CTANLoadOut, compilation] Error: called process '{0}' not found,".format(call_compile[1]),
            ...
            if verbose: print("[CTANLoadOut, compilation] Warning: LaTeX file '{0}' without content, no compilation".format(file_name))
    else:
        if verbose: print("[CTANLoadOut, compilation] Warning: LaTeX file '{0}' does not exist, no compilation".format(file_name))
...
def head():                                                       # function head: shows the called options
    ...
    if verbose:
        ...
        if ("-tout" in call) or ("--timeout" in call):
            print("  {0:5} {2:70} {1}".format("-tout", timeout, "(" + timeout_text + ")"))                                            # -tout
        ...
        if regeneration: print("[CTANLoadOut] Info: CTANLoad (Regeneration) is to be processed")
        if load:         print("[CTANLoadOut] Info: CTANLoad (Load)         is to be processed")
        if check:        print("[CTANLoadOut] Info: CTANLoad (Check)        is to be processed")
        if output:       print("[CTANLoadOut] Info: CTANOut                 is to be processed")
        if compile:      print("[CTANLoadOut] Info: LuaLaTeX and MakeIndex  are to be processed")
...
def main():                                                        # main function
    ...
    if statistics:                                                 # outputs the statistics
        ...
        print("program | version | date:".ljust(left + 3), programname_ext, "|", programversion, "|", programdate)

        print("---")
        print("total time (CTANLoadOut): ".ljust(left + 3), str(round(endtotal-starttotal, 2)).rjust(pp), "s")
        print("process time (CTANLoadOut): ".ljust(left + 3), str(round(endprocess-startprocess, 2)).rjust(pp), "s")
